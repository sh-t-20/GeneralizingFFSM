...
import ir.ac.ut.fml2.SCCFinder;
import net.automatalib.automata.fsa.impl.FastNFA;
import net.automatalib.automata.fsa.impl.FastNFAState;
import uk.le.ac.fts.FtsUtils;

public class FSMVisualizer3 {
	...
	public FSMVisualizer3() {
	}

	public void visualize(FastNFA<String> fsm, DataManagerFactory3 data_manager_factory,
			SimpleConfiguration product_config_1) {
		set_fsm(fsm);
		set_reachable_states_by_bfs();
		set_scc_check(data_manager_factory);
		save_fsm_kiss(data_manager_factory, product_config_1);
	...
	private void set_scc_check(DataManagerFactory3 data_manager_factory) {
	...
	private void save_fsm_kiss(DataManagerFactory3 data_manager_factory, SimpleConfiguration product_config_1) {
		StringBuilder buffer = new StringBuilder();

		String header = FtsUtils.getInstance().simpleConfigurationToString(product_config_1);
			...
			for (String in : fsm_.getLocalInputs(state)) {
				...
				for (FastNFAState neighbors_it : neighbors) {
					if (reachable_states_.get(neighbors_it) == false || reachable_states_.get(state) == false)
						continue;
					if (should_one_scc_check_ == true
							&& (is_in_root_scc_.get(neighbors_it) == false || is_in_root_scc_.get(state) == false))
						continue;
	...
	private void save_fsm_graph_view(DataManagerFactory3 data_manager_factory) {
		StringBuilder buffer = new StringBuilder();
